/*
    ...
    problem-> X_Total_Shapes(good).cpp
...
void dfs(int i,int j,vector<string> &a,int n,int m,vector<vector<bool> > &vis)
{
    if(i<0 || i>=n || j<0  || j>=m  || a[i][j]=='O'  || vis[i][j]==true)
    {
        return ;
    }
...
int Shape(vector<string> &a,int n,int m) {
