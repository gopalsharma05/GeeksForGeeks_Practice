/*
    ...
    problem-> Children_Sum_Parent(easy).cpp
...
Your Task:
You don't need to read input or print anything. Your task is to complete the function isSumProperty() that takes the root Node of the Binary Tree as input and returns 1
...
{
    if(!root)     // if we reach at NULL node
    return true;
    ...
    return(root->data==rl+rr && isSumProperty(root->left) &&  isSumProperty(root->right));
